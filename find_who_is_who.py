...
class Solver:
    ...
    def solve(self, node_obj):
        if len(self.solved_nodes) > 2:
            return self._solve_if_solved_nodes_are_greater_than_three(node_obj)
        else:
            self._solve_two_first_nodes()
            self.solve(node_obj)

    def _solve_if_solved_nodes_are_greater_than_three(self, node_obj):
        """
        find x and y for
        ...
        =>y = ((distance_to_mother - distance_to_node0)*x1 - (distance_to_mother - distance_to_node1)*x0)/2(x1y0-x0y1)
        =>x = ((distance_to_mother - distance_to_node0)*y1 - (distance_to_mother - distance_to_node1)*y0)/2(y1x0-x1y0)
        :return:
        """
        if [node_obj, node_obj._return_pos()] not in self.solved_nodes:
            distance_to_mother = node_obj.send_tone(self.solved_nodes[0][0])
            distance_to_node0 = node_obj.send_tone(self.solved_nodes[1][0])
            distance_to_node1 = node_obj.send_tone(self.solved_nodes[2][0])
            ...
            self.solved_nodes.append([node_obj, {'x': x_pos, 'y': y_pos}])

    def _solve_two_first_nodes(self):
        """
...
if __name__ == '__main__':
    ...
    factory.create_nodes(number_of_nodes)
    solver = Solver(factory.get_mother())
    [obj_1, obj_2] = factory.get_nodes_number(2)
    solver._cheating(obj_1,obj_2)
    for node_obj in factory.get_nodes():
